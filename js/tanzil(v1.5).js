...
function showPageText(page) {
    ...
    for (i = 0; i < list.length; i++) {
        ...
        if (currTab == 'trans') {
            ...
            thisAya += '<span class="ayaNumber">(<a href="{0}">{1}</a>)</span>'.format(domain + '#' + History.getHash({
                sura: sura,
                aya: aya
...
function getFontID() {
    ...
    if (id == 'def') {
        ...
        if ((isMac || isIOS) && isSafari && !isChrome)
            id = 'scheherazade';
        if (isChrome && isUthmani())
            id = 'scheherazade';
    }
    return id;
}
...
function fontWidth(fontName, text) {
    ...
    if (text == 2)
        text = 'In the name of Allah, بسم الله الرحمن الرحيم';
...
var Dialog = {
    ...
    setButtons: function() {
        ...
        for (var button in buttons) {
            ...
            $('<span/>').addClass('button').attr('id', id).text(_(button)).click(buttons[button]).disableTextSelect().appendTo($('.dbox-footer'));
        }
    },
